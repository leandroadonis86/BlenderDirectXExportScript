...
class DirectXExporter:
    def __init__(self, Config, context):
        ...
        if self.Config.CoordinateSystem == 'LEFT_HANDED':
            if self.Config.ExportAnimation == True:
                self.SystemMatrix *= Matrix.Scale(-1, 4, Vector((1, 0, 0)))
                self.SystemMatrix *= Matrix.Scale(-1, 4, Vector((0, 1, 0)))
            if self.Config.ExportAnimation == False:
                self.SystemMatrix *= Matrix.Scale(-1, 4, Vector((0, 0, 1)))	
                self.SystemMatrix *= Matrix.Scale(-1, 4, Vector((1, 0, 0)))
				
        if self.Config.UpAxis == 'Y':
            self.SystemMatrix *= Matrix.Rotation(radians(-90), 4, 'X')
            self.SystemMatrix *= Matrix.Rotation(radians(90), 4, 'Z')
        ...
        if self.Config.ExportAnimation:
            ...
            if self.Config.ExportActionsAsSets:
                self.AnimationWriter = SplitSetAnimationWriter(self.Config, self, AnimationGenerators)
            else:
                self.AnimationWriter = JoinedSetAnimationWriter(self.Config, self, AnimationGenerators)
            self.Log("Done")
    ...
    def __WriteHeader(self):
        ...
        if self.Config.IncludeFrameRate:
            ...
            self.File.Write(" <9e415a43-7ba6-4a73-8743-b73d47e88476>\n")
            self.File.Write(" DWORD AnimTicksPerSecond;\n}\n\n")
        ...
        if self.Config.ExportSkinWeights:
            ...
            self.File.Write(" <3cf169ce-ff7c-44ab-93c0-f78f62d172e2>\n")
            self.File.Write(" WORD nMaxSkinWeightsPerVertex;\n")
            self.File.Write(" WORD nMaxSkinWeightsPerFace;\n")
            self.File.Write(" WORD nBones;\n}\n\n")
        ...
        if self.Config.ExportSkinWeights:
            self.File.Write("template SkinWeights {\n")
            self.File.Write(" <6f0d123b-bad2-4167-a0d0-80224f25fabb>\n")
            self.File.Write(" STRING transformNodeName;\n")
            self.File.Write(" DWORD nWeights;\n")
            self.File.Write(" array DWORD vertexIndices[nWeights];\n")
            self.File.Write(" array FLOAT weights[nWeights];\n")
            self.File.Write(" Matrix4x4 matrixOffset;\n}\n\n\n")

    # Start the Root frame and write its transform matrix
    def __OpenRootFrame(self):
        self.File.Write("Frame SCENE_ROOT {\n")
        self.File.Indent()
        ...
        if not self.Config.ExportActionsAsSets:
            ...
            for Object in self.ExportList:
                if Object.BlenderObject.type == 'ARMATURE':
                    Generators.append(ArmatureAnimationGenerator(self.Config, None, Object))
                else:
                    Generators.append(GenericAnimationGenerator(self.Config, None, Object))
        # Otherwise,
        else:
            ...
            if len(ActionlessObjects):
                Generators.append(GroupAnimationGenerator(self.Config, "Default_Action", ActionlessObjects))
...
class MeshExportObject(ExportObject):
    ...
    def __WriteMesh(self, Mesh):
        ...
        if (self.Config.ExportUVCoordinates and Mesh.uv_textures) or \
            ...
            (self.Config.ExportSkinWeights) or (self.Config.ExportVertexDuplicates):
...
class AnimationWriter:
    ...
    # Writes all AnimationSets.  Implementations probably won't have to override this method.
        ...
        for Set in self.AnimationSets:
            ...
            for Generator in Set.AnimationGenerators:
                for CurrentAnimation in Generator.Animations:
                    self.Exporter.Log("Writing animation of {}".format(CurrentAnimation.SafeName))
                    ...
                    for Frame, Key in enumerate(CurrentAnimation.RotationKeys):
                        self.Exporter.File.Write("{};4;{:9f},{:9f},{:9f},{:9f};;".format(Frame, -Key[0], Key[1], Key[2], Key[3]))
                    ...
                    for Frame, Key in enumerate(CurrentAnimation.ScaleKeys):
                        self.Exporter.File.Write("{};3;{:9f},{:9f},{:9f};;".format(Frame, Key[0], Key[1], Key[2]))
            ...
            self.Exporter.Log("Done writing animation set {}".format(Set.SafeName))
...
class Util:
    ...
    def WriteMatrix(File, Matrix):
        File.Write("{:9f},{:9f},{:9f},{:9f},{:9f},{:9f},{:9f},{:9f},{:9f},{:9f},{:9f},{:9f},{:9f},{:9f},{:9f},{:9f};;\n" \
        .format(Matrix[0][0], Matrix[1][0], Matrix[2][0], Matrix[3][0], \
